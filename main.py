    ...
    if button_id in ['selectinvent', 'selectall']:
        ...
        if inventory['type'] in ['animals']:
            ...
            if button_id in ['selectall']:
                ...
                sellall = InlineKeyboardButton(f"🔘 {str(int(sum([d['cost'] for d in inventory])*discont))} 🔪 Под нож", callback_data=f"{button_parent['id']}|getcryptoall|{stepinventory}|{inventory['id']}")
