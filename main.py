...
def get_raid_plan(raid_date, goat):
    tz = config.SERVER_MSK_DIFF
    plan_for_date = f'План рейдов на {time.strftime("%d.%m.%Y %H:%M", time.gmtime( (datetime.fromtimestamp(raid_date) + timedelta(seconds=tz.second, minutes=tz.minute, hours=tz.hour)).timestamp() ))}\n🐐<b>{goat}</b>\n\n'
    find = False
    time_str = None
