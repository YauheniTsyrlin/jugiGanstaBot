    ...
    'commission':
    {
        ...
        'buttons': [
            ...
            },
            {
                'id': 'farm',
                'name': '🐐🌳 Ферма',
                'description':'🐐🌳 Здесь можно заниматься выращиванием 🐮 домашних животных и 🌳 садоводством.\n\n ☣️ Внимание! На одной ферме можно содержать не более 100 особей, в том числе их яиц, личинок, зародышей и пр. ☣️',
                'buttons': []
            }
        ]
    }
...
def isTimeFarmOver(userIAm, forward_date, chat):
    ...
    if storage > skill['min']:
        # Сила навыка = 0, если [Текущее значение] < [Min, с которого действует навык] 
        # Сила навыка = [0, 1]
        # Сила навыка = ([Текущее значение] - [Min, с которого действует навык]) / ([Max навыка] - [Min, с которого действует навык]) 
        # 
        # Дополнительное время фарма  =  округлени_до_целого([Сила навыка] * 10) = Х минут от 0 до 10.
        # К примеру, 
        #   [Min, с которого действует навык] = 50
        #   [Max] = 200
        #   [Текущее значение] = 100
        # Дополнительное время фарма = (100 - 50 / 200 - 50)  * 10 = 3 мин
        # Дополнительное время фарма = (150 - 50 / 200 - 50)  * 10 = 6 мин
        # Дополнительное время фарма = (190 - 50 / 200 - 50)  * 10 = 9 мин
...
def callback_query(call):
    ...
    if call.data.startswith('pinraid_band'):
        ...
        for user in list(filter(lambda x : x.getBand() == band, USERS_ARR)):
            ...
            logger.info(f"pinr_usr_{raid_date.timestamp()}_{band}_{user.getLogin()}")
            logger.info(len(f"pinr_usr_{raid_date.timestamp()}_{band}_{user.getLogin()}"))

            buttons.append(InlineKeyboardButton(f"{planed_location_str}{user.getNameAndGerb()}", callback_data=f"pinr_usr_{raid_date.timestamp()}_{band}_{user.getLogin()}"))
        
        all_banditos=InlineKeyboardButton(f"👥 Все бандиты", callback_data=f"pinr_usr_{raid_date.timestamp()}_{band}_allbanditos")
    ...
    if call.data.startswith('pinr_usr'):
        #   0      1               2             3       4
        # pinr_usr_{raid_date.timestamp()}_{band}_allbanditos
